    ...
    <SafeAreaView style={[styles.container, { backgroundColor: t.background }]} edges={['bottom']}>
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <Box style={[styles.headerSection, { backgroundColor: t.cardBackground }]}>
          ...
          {/* Name + verified */}
          <Box style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 4 }}>
          ...
          <Box style={styles.statsRow}>
            ...
            <Box style={styles.statItem}>
              <Text style={[styles.statNumber, { color: t.text }]}>
                {userPosts.reduce((sum, post) => sum + (Number(post.votes) || 0), 0)}
              </Text>
            ...
            <Box style={styles.statItem}>
              <Text style={[styles.statNumber, { color: t.text }]}>
                {userPosts.reduce((sum, post) => sum + (Number(post.comments) || 0), 0)}
              </Text>
          ...
          {userData.created_at && (
            <Text style={[styles.memberSince, { color: t.secondaryText }]}>
              Member since {formatTime(userData.created_at, translate)}
            </Text>
          )}
        </Box>
        ...
        <Box style={[styles.tabContainer, { backgroundColor: t.cardBackground, borderBottomColor: t.border }]}>
          <Pressable
            style={[styles.tab, selectedTab === 'Posts' && { borderBottomColor: t.accent, borderBottomWidth: 2 }]}
            onPress={() => setSelectedTab('Posts')}
          >
            <Text style={[styles.tabText, { color: selectedTab === 'Posts' ? t.accent : t.secondaryText }]}>
              Posts ({userPosts.length})
            </Text>
          </Pressable>
          <Pressable
            style={[styles.tab, selectedTab === 'About' && { borderBottomColor: t.accent, borderBottomWidth: 2 }]}
            onPress={() => setSelectedTab('About')}
          >
            <Text style={[styles.tabText, { color: selectedTab === 'About' ? t.accent : t.secondaryText }]}>
              About
            </Text>
          </Pressable>
        </Box>
        ...
        {selectedTab === 'About' && (
          <Box style={[styles.aboutContainer, { backgroundColor: t.cardBackground }]}>
            ...
            <Text style={[styles.aboutText, { color: t.secondaryText, fontStyle: userData.about ? 'normal' : 'italic' }]}>
            ...
            {userData.phone && (
              <Box style={styles.infoRow}>
                ...
                <Text style={[styles.infoText, { color: t.secondaryText }]}>{userData.phone}</Text>
              </Box>
            )}
...
const styles = StyleSheet.create({
  ...
  avatarContainer: { position: 'relative', marginBottom: 16, width: 120, height: 120 },
  ...
  aboutContainer: { margin: 16, padding: 20, borderRadius: 12 },
